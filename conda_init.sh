#!/usr/bin/env bash
# bootstrap_42ai.sh — Installe/initialise Miniconda + env de travail, idempotent.
# Usage: ./bootstrap_42ai.sh [--env NAME] [--py 3.7] [--force] [--purge] [--no-open]
set -Eeuo pipefail

# ------------------------------- Defaults ------------------------------------
MINICONDA_PREFIX="${MINICONDA_PREFIX:-$HOME/goinfre/miniconda3}"
INSTALLER="${INSTALLER:-${TMPDIR:-/tmp}/miniconda.sh}"
ENV_NAME="${ENV_NAME:-${USER}-42ai}"
